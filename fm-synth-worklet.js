...
class FMSynthProcessor extends AudioWorkletProcessor {
  constructor(options) {
    ...
    this.operatorCount = 6;
    this.patch = normalizePatch(options?.processorOptions?.patch);
    ...
    this.port.onmessage = (event) => {
      ...
      switch (msg.type) {
        case 'setPatch':
          this.patch = normalizePatch(msg.patch);
          break;
        case 'noteOn':
        case 'noteOff':
        ...
        case 'allNotesOff':
          ...
          for (const voice of this.voices) voice.forceRelease(this.patch);
          break;
        case 'panic':
          this.eventQueue.length = 0;
          for (const voice of this.voices) voice.hardStop();
          break;
        default:
          break;
      }
  ...
  noteOn(event) {
    ...
    voice.start(event.noteId, event.midiNote, event.frequency, event.velocity, this.patch, this.voiceAgeCounter++);
  }
  ...
  noteOff(event) {
    ...
    for (const voice of this.voices) {
      if (voice.active && voice.midiNote === event.midiNote) {
  ...
  process(_inputs, outputs) {
    ...
    for (let i = 0; i < left.length; i += 1) {
      ...
      let mixed = 0;
      for (const voice of this.voices) {
        if (voice.active) mixed += voice.process(this.patch);
      }

      const out = Math.tanh(mixed * 0.9);
      left[i] = out;
